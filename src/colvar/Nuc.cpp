...
namespace PLMD{
  namespace colvar{
    ...
    */
    ...
    PLUMED_REGISTER_ACTION(Nuc,"NUC")

    void Nuc::registerKeywords(Keywords& keys){
    ...
    Nuc::Nuc(const ActionOptions&ao):
    ...
    double Nuc::sigmon(double z, double Coff){
      double sig;
      if( z < -Coff){
	sig=0.0;
      }else if(z > Coff){
    ...
    double Nuc::sigmoff(double z, double Coff){
      double sig;
      if( z < -Coff){
	sig=1.0;
      }else if(z > Coff){
    ...
    double Nuc::dsig(double z, double Coff){
      double dsig;
      if(fabs(z) > Coff){
	dsig=0.0;
      }else{
...
//    }
     ...
     // calculate the shift
      ...
      double  zleft=LBC[2]/2;
      double  zright=LBC[2]/2;
