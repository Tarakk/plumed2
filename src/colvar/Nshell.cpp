...
namespace PLMD{
  namespace colvar{
    ...
    */
    ...
    void Nshell::registerKeywords(Keywords& keys){
    ...
    Nshell::Nshell(const ActionOptions&ao):
    ...
    double Nshell::sigmoff(double z, double Coff){
      double sig;
      if( z < -Coff){
	sig=1.0;
      }else if(z > Coff){
    ...
    double Nshell::dsig(double z, double Coff){
      double dsig;
      if(fabs(z) > Coff){
	dsig=0.0;
      }else{
	...
	for(int i=rank; i<N_st; i+=stride){
	  ...
	  if(asymm>=0){
	    ...
	    dr=(-dsig(zr,co_f)-dsig(zr+LBC[2]/w_force,co_f))/w_force;
          //log.printf("check distance: %lf %lf %lf\n",zin,zout,zr);
	  }
