...
namespace PLMD{
  namespace colvar{
    ...
    /*
      ...
      WARNING!!! The derivative contains only the outer boundary terms, 
      the resulting bias force is non-conservative!!! 
      NO METADYNAMICS, use only for CmuMD!!!
    */
    //+ENDPLUMEDOC
   ...
   //
   // Inner boundary derivatives have to be included in order to use NSHELLSP as a CV!
   //

   // only outer boundary derivatives, no BIAS, only for CuMD 
 ...
 // calculator
  void NshellSp::calculate()    
  {
    ...
    //CR shell size
    //double D_CR=D_OUT-D_IN;

    //CR volume 
      double VCR;
      VCR=(4.0/3.0)*pi*(D_CR*D_CR*D_CR); //CR volume 
  ...
  //Evaluate concentration and derivatives
    //if isolute is true C counts the solute molecules, else the solvent ones
      n_CR=0.0;
      double d,din,dout,dF,n_rx,dfunc,dr; 
    ...
    //Solute and Solvent position matrix allocation 
      ...
      if(N_st == 0){  //if solvent species is restrained
        ...
        // Distance of each molecule from the box center
           ...
           d = id[i].modulo2();
           double inv_d=1.0/d;
      //log.printf("check distance: \td %lf\n",d);

           din=(d-D_IN)/w_in;
           dout=(d-D_OUT)/w_out;
           dF=(din-D_F)/w_force;

      //log.printf("check distance: \tdin %lf\tdout, %lf\tdF %lf\n",din,dout,dF);
         
        //With periodic image, sigma on at zout, off at zin
           n_rx=sigmon(din,co_in)*sigmoff(dout,co_out); //+sigmon(zin-LBC[2]/w_in,co_in)*sigmoff(zout-LBC[2]/w_out,co_out);
           dr=(-dsig(dF,co_f))/w_force;   //+dsig(zl-LBC[2]/w_force,co_f))/w_force;	 // spherical boundary, no periodic image

           dfunc=dr;
           n_CR+=n_rx; //update CV (for now this is the number of molcules)
          
         //include the derivatives here
           k=N_st+i*Na_sv_permol; //atom counter (just the derivatives with respect to "com" atom coordinates)	   
           for (unsigned int ix=0; ix<3; ix++) {          // this part needs to be finalized
             deriv[k][ix]  +=  dfunc*inv_d*(id[k][0]+id[k][1]+id[k][2])/VCR; // Derivative of ds/dx = (x/s)
           }
         }
	vector<Vector>().swap(solv_x);
        ...
        //with periodic image, sigma on at zout, off at zin
           n_rx=sigmon(din,co_in)*sigmoff(dout,co_out); //+sigmon(zin-LBC[2]/w_in,co_out)*sigmoff(zout-LBC[2]/w_out,co_out);
           dr=(-dsig(dF,co_f))/w_force; //+dsig(zl-LBC[2]/w_force,co_f))/w_force; // spherical boundary, no periodic image	
         ...
         //include the derivatives here
           k=i*Na_st_permol; //atom counter (just the derivatives with respect to "com" atom coordinates)
           for (unsigned int ix=0; ix<3; ix++) {          // this part needs to be finalized
             deriv[k][ix]  +=  dfunc*inv_d*(id[k][0]+id[k][1]+id[k][2])/VCR; // Derivative of dzin/dx, dzin/dy, & dzin/dz
           }
	}
